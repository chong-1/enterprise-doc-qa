"""RAG 问答 API（非流式 + SSE 流式 + Agent 模式 + 对话记忆 + 审计）。"""
...
async def ask_question(
    ...
    _: Annotated[User, Depends(require_kb_role("viewer"))] = None,
):
    """向知识库提问（viewer 及以上）。支持对话记忆、Agent 模式、SSE 流式。"""
    kb = await db.get(KnowledgeBase, kb_id)
    if kb is None:
        raise NotFoundError(f"知识库 {kb_id} 不存在")

    # 问答审计（精简版：问题截断到 100 字）
    await audit_service.log_action(
        db, user, "qa:query", "qa", kb_id,
